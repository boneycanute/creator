...
export const AgentNavigationHint: React.FC<AgentNavigationHintProps> = ({
  ...
  const { questions } = useAgentFormStore();
  ...
  }, [isFirstQuestion, currentQuestion.type]);

  const handleNext = () => {
    // Don't allow navigation for loading screens
    if (currentQuestion.type === 'loading') return;
  ...
  // Don't show navigation for loading screens
  if (currentQuestion.type === 'loading') {
    return (
      <motion.div 
          ...
          {isMobile ? (
            ...
            <div className="flex items-center justify-between gap-4">
              {!isFirstQuestion && (
                ...
                >
                  <motion.div
                    className={`key-container ${escPressed ? 'key-pressed' : ''}`}
                    animate={
                      escPressed 
